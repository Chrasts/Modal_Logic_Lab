...
export const campaignTracks: readonly CampaignTrack[] = [
  ...
  {
    id: 'axiom-breaker', title: 'Axiom Breaker',
    description: 'Falsify modal principles while preserving the required frame property.',
    levels: [
      {
        id: 'axiom-breaker-four-reflexive', chapter: 'Axiom Breaker', title: 'Break axiom 4', concept: 'Reflexivity without transitivity',
        learningObjective: 'Connect failure of axiom 4 with a missing transitive shortcut on a reflexive frame.',
        instruction: 'Make □p → □□p false at w0 while keeping the frame reflexive and using at most 3 worlds.',
        briefing: 'Reflexivity alone does not force the transitive relation needed by axiom 4.',
        hints: ['For an implication to fail, its antecedent must be true and its consequent false.', 'Build a two-step route from w0 whose last world makes p false.', 'Keep every reflexive loop, but omit the shortcut from w0 to the final world.'],
        successDebrief: 'This countermodel shows a reflexive frame where the missing transitive shortcut lets □p hold at w0 while □□p fails there. One countermodel refutes the principle on the class of reflexive frames.',
        formula: '□p → □□p', scope: 'pointed', targetTruth: false, evaluationWorld: 'w0',
        worlds: [{ id: 'w0', atoms: '', position: { x: 245, y: 130 } }], edges: [], frameRules: { reflexive: 'validate' },
        constraints: { maximumWorlds: 3, requiredProperties: ['reflexive'] }, editable: ['worlds', 'valuations', 'edges'],
        workspacePresentation: { worlds: true, valuations: true, edges: true, visibleConstraints: ['The frame must be reflexive.', 'Use at most 3 worlds.'] },
        referenceSolution: { evaluationWorld: 'w0', worlds: [{ id: 'w0', atoms: 'p', position: { x: 70, y: 130 } }, { id: 'w1', atoms: 'p', position: { x: 260, y: 130 } }, { id: 'w2', atoms: '', position: { x: 450, y: 130 } }], edges: [{ from: 'w0', to: 'w0' }, { from: 'w1', to: 'w1' }, { from: 'w2', to: 'w2' }, { from: 'w0', to: 'w1' }, { from: 'w1', to: 'w2' }] },
      },
      {
        id: 'axiom-breaker-five-serial', chapter: 'Axiom Breaker', title: 'Break axiom 5', concept: 'Seriality without Euclideanness',
        learningObjective: 'Distinguish seriality from the Euclidean property associated with axiom 5.',
        instruction: 'Make ◇p → □◇p false at w0 while keeping the frame serial and using at most 3 worlds.',
        briefing: 'Seriality gives every world a successor, but it does not make successors share the same possibilities.',
        hints: ['The antecedent needs one accessible p-world.', 'The consequent fails if another successor has no route to any p-world.', 'Give every world a successor while keeping one branch away from p.'],
        successDebrief: 'The serial frame supplies successors everywhere, yet one successor of w0 cannot reach a p-world. This separates seriality from the Euclidean structure behind axiom 5.',
        formula: '◇p → □◇p', scope: 'pointed', targetTruth: false, evaluationWorld: 'w0',
        worlds: [{ id: 'w0', atoms: '', position: { x: 245, y: 130 } }], edges: [], frameRules: { serial: 'validate' },
        constraints: { maximumWorlds: 3, requiredProperties: ['serial'] }, editable: ['worlds', 'valuations', 'edges'],
        workspacePresentation: { worlds: true, valuations: true, edges: true, visibleConstraints: ['The frame must be serial.', 'Use at most 3 worlds.'] },
        referenceSolution: { evaluationWorld: 'w0', worlds: [{ id: 'w0', atoms: '', position: { x: 245, y: 60 } }, { id: 'w1', atoms: 'p', position: { x: 90, y: 230 } }, { id: 'w2', atoms: '', position: { x: 400, y: 230 } }], edges: [{ from: 'w0', to: 'w1' }, { from: 'w0', to: 'w2' }, { from: 'w1', to: 'w1' }, { from: 'w2', to: 'w2' }] },
      },
    ],
  },
  {
    id: 'model-budget', title: 'Model Budget',
    description: 'Meet a semantic objective within explicit world and relation budgets.',
    levels: [
      {
        id: 'model-budget-split-possibility', chapter: 'Model Budget', title: 'Split possibility', concept: 'Two possibility witnesses within a budget',
        learningObjective: 'Construct accessible p and non-p cases within a small relation budget.',
        instruction: 'Make ◇p ∧ ◇¬p true at w0 using at most 3 worlds and 2 explicit relations.',
        hints: ['Each diamond may use a different witness.', 'One successor should make p true and another should make p false.', 'Connect w0 directly to the two contrasting worlds.'],
        successDebrief: 'The objective is satisfied within the budget because w0 reaches both a p-case and a non-p-case.',
        formula: '◇p ∧ ◇¬p', scope: 'pointed', targetTruth: true, evaluationWorld: 'w0',
        worlds: [{ id: 'w0', atoms: '', position: { x: 245, y: 130 } }], edges: [], constraints: { maximumWorlds: 3, maximumEdges: 2 }, editable: ['worlds', 'valuations', 'edges'],
        workspacePresentation: { worlds: true, valuations: true, edges: true, visibleConstraints: ['Use at most 3 worlds.', 'Use at most 2 explicit relations.'] },
        referenceSolution: { evaluationWorld: 'w0', worlds: [{ id: 'w0', atoms: '', position: { x: 245, y: 60 } }, { id: 'w1', atoms: 'p', position: { x: 90, y: 230 } }, { id: 'w2', atoms: '', position: { x: 400, y: 230 } }], edges: [{ from: 'w0', to: 'w1' }, { from: 'w0', to: 'w2' }] },
      },
      {
        id: 'model-budget-two-steps', chapter: 'Model Budget', title: 'Two steps away', concept: 'Acyclic two-step reachability',
        learningObjective: 'Build genuine two-step modal reachability without a loop or cycle.',
        instruction: 'Make ◇◇p true at w0 on an acyclic frame using at most 3 worlds and 2 explicit relations.',
        hints: ['The outer diamond needs a successor that satisfies the inner diamond.', 'The inner diamond needs its own accessible p-world.', 'Use a chain from w0 through one intermediate world to a p-world.'],
        successDebrief: 'The p-world is reachable in two steps, and the construction stays acyclic and within both budgets.',
        formula: '◇◇p', scope: 'pointed', targetTruth: true, evaluationWorld: 'w0',
        worlds: [{ id: 'w0', atoms: '', position: { x: 245, y: 130 } }], edges: [], frameRules: { acyclic: 'validate' },
        constraints: { maximumWorlds: 3, maximumEdges: 2, requiredProperties: ['acyclic'] }, editable: ['worlds', 'valuations', 'edges'],
        workspacePresentation: { worlds: true, valuations: true, edges: true, visibleConstraints: ['The frame must be acyclic.', 'Use at most 3 worlds.', 'Use at most 2 explicit relations.'] },
        referenceSolution: { evaluationWorld: 'w0', worlds: [{ id: 'w0', atoms: '', position: { x: 70, y: 130 } }, { id: 'w1', atoms: '', position: { x: 260, y: 130 } }, { id: 'w2', atoms: 'p', position: { x: 450, y: 130 } }], edges: [{ from: 'w0', to: 'w1' }, { from: 'w1', to: 'w2' }] },
      },
    ],
  },
  {
